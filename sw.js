...
var CACHE_VERSION = 'dh-v2.2-20260317'; // Update this string on every deploy to bust stale caches
...
self.addEventListener('activate', function(e) {
  e.waitUntil(
    caches.keys().then(function(keys) {
      ...
      // Notify clients a new version is available — let them choose to reload
    ...
    e.respondWith(
      fetch(req).then(function(resp) {
        if (resp.ok) {
          ...
          var respToCache = resp.clone();
          caches.open(PAGE_CACHE).then(function(c) {
            c.put(req, respToCache);
            // Evict oldest entries — keep max 30 pages to prevent unbounded cache growth
            c.keys().then(function(keys) {
              if (keys.length > 30) {
                keys.slice(0, keys.length - 30).forEach(function(k) { c.delete(k); });
              }
            });
          });
        }
