...
(function (global) {
  ...
  function _flush() {
    ...
    if (navigator.sendBeacon) {
      ...
      fetch(url + '?action=logErrors', {
        ...
        // If server returns non-ok (e.g. GAS quota exceeded), re-queue for retry
        if (!r.ok && r.status !== 0) {
      ...
      return fetch(url, fetchOpts)
        ...
        .catch(function (err) {
          ...
          if (circuit.failCount >= 3 && !circuit.open) {
            circuit.open = true;
            var _ckm = url.match(/[?&]action=([^&]+)/);
            var _circuitKey = _ckm ? _ckm[1] : (url.split('/')[2] || 'default');
      ...
      if (headerMount && headerMount.childElementCount === 0) {
        ...
        try {
          if (typeof renderHeader === 'function') {
            ...
            // Re-initialize all nav JS that depends on the rendered DOM elements
            ...
            if (typeof Wishlist !== 'undefined' && Wishlist.updateUI) Wishlist.updateUI();
          }
        } catch (e) { /* silent */ }
      }
  ...
  var DHPerf = (function () {
    ...
    // Navigation timing (page load) — uses Navigation Timing Level 2 API
    function _checkNavTiming() {
      if (!performance) return;
      global.addEventListener('load', function () {
        setTimeout(function () {
          // Prefer Navigation Timing Level 2 (modern, non-deprecated)
          ...
          if (entries && entries.length) {
            ...
            if (total > 5000) {
              _enqueue('PERF_LCP', 'perf', 'Slow page load: ' + Math.round(total) + 'ms', {
                ttfb: Math.round(ttfb)
              });
            }
          } else if (performance.timing) {
            // Fallback to Level 1 for older browsers
            var t     = performance.timing;
            var total = t.loadEventEnd - t.navigationStart;
            var ttfb  = t.responseStart - t.navigationStart;
            if (total > 5000) {
              _enqueue('PERF_LCP', 'perf', 'Slow page load (L1): ' + total + 'ms', { ttfb: ttfb });
            }
          }
        }, 0);
      });
